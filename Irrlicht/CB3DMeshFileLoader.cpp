		...
		if (flags & 1)
		{
			...
			if ((oldPosKey!=0) && (oldPos[0]==oldPos[1]))
			{
				...
				else
				{
					...
					oldPosKey->frame = (f32)frame-1;
					oldPos[1].set(oldPosKey->value.set(pos));
				}
			}
			else if (oldPosKey==0 && isFirst[0])
			{
				oldPosKey=AnimatedMesh->addPositionKey(inJoint);
				oldPosKey->frame = (f32)frame-1;
				oldPos[0].set(oldPosKey->value.set(data[0], data[1], data[2]));
				oldPosKey=0;
				isFirst[0]=false;
			}
			else
			{
				...
				oldPosKey=AnimatedMesh->addPositionKey(inJoint);
				oldPosKey->frame = (f32)frame-1;
				oldPos[1].set(oldPosKey->value.set(data[0], data[1], data[2]));
			}
		}
		if (flags & 2)
		{
			...
			if ((oldScaleKey!=0) && (oldScale[0]==oldScale[1]))
			{
				...
				else
				{
					...
					oldScaleKey->frame = (f32)frame-1;
					oldScale[1].set(oldScaleKey->value.set(scale));
				}
			}
			else if (oldScaleKey==0 && isFirst[1])
			{
				oldScaleKey=AnimatedMesh->addScaleKey(inJoint);
				oldScaleKey->frame = (f32)frame-1;
				oldScale[0].set(oldScaleKey->value.set(data[0], data[1], data[2]));
				oldScaleKey=0;
				isFirst[1]=false;
			}
			else
			{
				...
				oldScaleKey=AnimatedMesh->addScaleKey(inJoint);
				oldScaleKey->frame = (f32)frame-1;
				oldScale[1].set(oldScaleKey->value.set(data[0], data[1], data[2]));
			}
		}
		if (flags & 4)
		{
			...
			if ((oldRotKey!=0) && (oldRot[0]==oldRot[1]))
			{
				...
				else
				{
					...
					oldRotKey->frame = (f32)frame-1;
					oldRot[1].set(oldRotKey->value.set(data[1], data[2], data[3], data[0]));
				}
			}
			else if (oldRotKey==0 && isFirst[2])
			{
				...
				oldRotKey->frame = (f32)frame-1;
				// meant to be in this order since b3d stores W first
				oldRot[0].set(oldRotKey->value.set(data[1], data[2], data[3], data[0]));
				oldRotKey=0;
				isFirst[2]=false;
			}
			else
