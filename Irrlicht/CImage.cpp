...
:Data(0), Size(size), DataSize(0), Format(format), DeleteMemory(true), Levels(nullptr), LevelCount(0)
{
	initData();
}
...
: Data(0), Size(size), DataSize(0), Format(format), DeleteMemory(deleteForeignMemory), Levels(nullptr), LevelCount(0)
{
	if (ownForeignMemory)
	{
		Data = (u8*)0xbadf00d;
...
void CImage::initData()
{
    ...
    u32 depthPicht;
	IImage::getPitch(Size.Width, Size.Height, 1, Format, Pitch, depthPicht, 0);

    DataSize = IImage::getMemorySize(Size.Width, Size.Height, 1, Format);
...
void CImage::initializeMipLevels(u8 levelCount, core::dimension2d<u32>* sizes)
{
    ...
    for (int i = 0; i < levelCount; ++i)
    {
        ...
        Levels[i].DataSize = IImage::getMemorySize(sizes[i].Width, sizes[i].Height, 1, Format);
...
const core::dimension2d<u32>& CImage::getDimension(u8 miplevel) const
{
    if (miplevel > 0)
        return Levels[miplevel - 1].Size;
	return Size;
}
...
u32 CImage::getImageDataSizeInPixels(u8 miplevel) const
{
    if (miplevel > 0)
        return Levels[miplevel - 1].Size.Width * Levels[miplevel - 1].Size.Height;
...
{
	if (!target || !width || !height || IImage::isCompressedFormat(format))
		return;
