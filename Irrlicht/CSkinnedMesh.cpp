	...
	if (joint->UseAnimationFrom)
	{
		...
		if (PositionKeys.size())
		{
			...
			if (foundPositionIndex!=-1)
			{
				...
				{
					position = PositionKeys[foundPositionIndex].value;
				}
				else if (InterpolationMode==EIM_LINEAR)
				{
					...
					position = ((KeyB.value -KeyA.value)/(fd1+fd2))*fd1 + KeyA.value;
				}
			}
		}
		...
		if (ScaleKeys.size())
		{
			...
			if (foundScaleIndex!=-1)
			{
				...
				{
					scale = ScaleKeys[foundScaleIndex].value;
				}
				else if (InterpolationMode==EIM_LINEAR)
				{
					...
					const f32 fd1 = frame - KeyA.frame;
					const f32 fd2 = KeyB.frame - frame;
					scale = ((KeyB.value -KeyA.value)/(fd1+fd2))*fd1 + KeyA.value;
				}
			}
		}
		...
		if (RotationKeys.size())
		{
			...
			if (foundRotationIndex!=-1)
			{
				...
				{
					rotation = RotationKeys[foundRotationIndex].value;
				}
				else if (InterpolationMode==EIM_LINEAR)
				{
					...
					*/
					...
					rotation.slerp(KeyA.value, KeyB.value, t);
				}
			}
		}
	}
	...
	if (HasAnimation)
	{
		...
		for(i=0;i<AllJoints.size();++i)
		{
			...
			if (PositionKeys.size()>2)
			{
				...
				{
					if (PositionKeys[j].value == PositionKeys[j+1].value && PositionKeys[j+1].value == PositionKeys[j+2].value)
					{
			...
			if (ScaleKeys.size()>2)
			{
				...
				{
					if (ScaleKeys[j].value == ScaleKeys[j+1].value && ScaleKeys[j+1].value == ScaleKeys[j+2].value)
					{
			...
			if (RotationKeys.size()>2)
			{
				...
				{
					if (RotationKeys[j].value == RotationKeys[j+1].value && RotationKeys[j+1].value == RotationKeys[j+2].value)
					{
