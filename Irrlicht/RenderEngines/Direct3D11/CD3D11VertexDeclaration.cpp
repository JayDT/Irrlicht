...
irr::u32 irr::video::CD3D11VertexDeclaration::GetVertexPitch(irr::u8 inputSlot) const
{
    if (VertexDeclarationMap.empty())
        const_cast<CD3D11VertexDeclaration*>(this)->initialize();

    return VertexPitch[inputSlot];
}

void irr::video::CD3D11VertexDeclaration::initialize()
{
    VertexDeclarationMap.clear();
