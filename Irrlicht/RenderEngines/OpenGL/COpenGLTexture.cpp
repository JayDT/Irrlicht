...
COpenGLTexture::COpenGLTexture(const io::path& name, const core::dimension2d<u32>& size, ECOLOR_FORMAT format, COpenGLDriver* driver)
    : ITexture(name, E_TEXTURE_TYPE::e2D)
    , Driver(driver)
    , NumberOfMipLevels(0)
    , NumberOfArraySlices(0)
    , Image(0)
    , Pitch(0)
    , TextureName(0), InternalFormat(GL_RGBA), PixelFormat(GL_RGBA), PixelType(GL_UNSIGNED_BYTE)
    , KeepImage(false), MipLevelStored(0), MipmapLegacyMode(true),
    IsRenderTarget(false), AutomaticMipmapUpdate(false),
    ReadOnlyLock(false)
{
    Driver->testGLError();
    ...
    Pitch = TextureSize.Width * IImage::getBitsPerPixelFromFormat(ColorFormat) / 8;
    ...
    Driver->LastMaterial.TextureLayer[0].TextureWrapU = ETC_CLAMP_TO_EDGE;
    Driver->LastMaterial.TextureLayer[0].TextureWrapV = ETC_CLAMP_TO_EDGE;
    Driver->LastMaterial.TextureLayer[0].TextureWrapU = ETC_CLAMP_TO_EDGE;

    glTexImage2D(GL_TEXTURE_2D, 0, InternalFormat, TextureSize.Width, TextureSize.Height, 0, PixelFormat, PixelType, 0);
    Driver->testGLError();
...
void COpenGLTexture::regenerateMipMapLevels(IImage* image)
{
    ...
    if (hasMipMaps() && Image && Image->GetMipLevelCount())
    {
        for (u8 level = 1; level <= Image->GetMipLevelCount(); ++level)
        {
            const auto& Size = Image->getDimension(level);
            u32 RowBytes, depthPicht;
...

            {
                ...
                if (IImage::isCompressedFormat(ColorFormat))
                {
                    glBufferData(GL_PIXEL_UNPACK_BUFFER, Image->getImageDataSizeInBytes(level), NULL, GL_STATIC_COPY);

                    void* bufferPtr = glMapBufferRange(GL_PIXEL_UNPACK_BUFFER, 0, Image->getImageDataSizeInBytes(level), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
                    memcpy(bufferPtr, Image->lock(level), Image->getImageDataSizeInBytes(level));
                    glUnmapBuffer(GL_PIXEL_UNPACK_BUFFER);


                    glCompressedTexImage2D(GL_TEXTURE_2D, level, InternalFormat, Size.Width, Size.Height, 0, Image->getImageDataSizeInBytes(level) /*imageSize*/, 0);
                }
                else
                {
                    glBufferData(GL_PIXEL_UNPACK_BUFFER, Image->getImageDataSizeInBytes(level), NULL, GL_STATIC_COPY);
                    ...
                    glTexImage2D(GL_TEXTURE_2D, level, InternalFormat, Size.Width, Size.Height, 0, PixelFormat, PixelType, 0);
                }
        ...
        level = Image->GetMipLevelCount();
    }
    else
    {
        core::dimension2du size = TextureSize;
        level = 0;
        u8* target = new u8[Pitch * size.Height];
        ...
        do
        {
            ...
            Image->copyToScaling(target, size.Width, size.Height, Image->getColorFormat());
            ...
            glTexImage2D(GL_TEXTURE_2D, level, InternalFormat, size.Width, size.Height,
