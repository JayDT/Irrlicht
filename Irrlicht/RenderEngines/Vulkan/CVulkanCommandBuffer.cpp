    ...
    , mVertexBufferOffsetsTemp()
{
    ...
    mResources.reserve(4096);
    mImages.reserve(4096);
    mBuffers.reserve(4096);
}
...
VulkanCmdBuffer::~VulkanCmdBuffer()
{
    ...
    else if (mState != State::Ready)
    {
        ...
        {
            ResourceUseHandle& useHandle = entry->mUseHandle;
...
void VulkanCmdBuffer::submit(VulkanQueue* queue, uint32_t queueIdx, uint32_t syncMask)
{
    ...
    for (auto& entry : mBuffers)
    {
        ...
        if (currentQueueFamily != (uint32_t)-1 && currentQueueFamily != mQueueFamily)
        {
            ...
            barrier.srcAccessMask = resource->mBufferInfo.accessFlags;
            barrier.dstAccessMask = resource->mBufferInfo.accessFlags;
    ...
    for (auto& entry : mResources)
    {
        ...
        useHandle.used = true;
        entry->notifyUsed(mGlobalQueueIdx, mQueueFamily, useHandle.flags);
    }
...
void VulkanCmdBuffer::reset()
{
    ...
    if (wasSubmitted)
    {
        ...
        {
            ResourceUseHandle& useHandle = entry->mUseHandle;
            entry->mUseHandle.mPoolMask &= ~1;
            assert(useHandle.used);

            entry->notifyDone(mGlobalQueueIdx, useHandle.flags);
        }
...
void VulkanCmdBuffer::setRenderTarget(const video::IRenderTarget* rt, uint32_t readOnlyFlags, RenderSurfaceMaskBits loadMask, bool LifeTimeBound)
{
    ...
    VulkanFramebuffer* newFB = rt ? static_cast<const CVulkanRenderTarget*>(rt)->getFrameBuffer() : nullptr;
    ...
    if (mFramebuffer != nullptr)
    {
        ...
        for (uint32_t i = 0; i < numColorAttachments; i++)
        {
            ...
            for (uint32_t j = 0; j < fbAttachment.image->mImageInfo.numSubresourceInfos; j++)
            {
                VulkanImageSubresource* subImage = fbAttachment.image->getSubresource(0, fbAttachment.image->mImageInfo.subresourceInfoIdx + i);
                ImageSubresourceInfo& entry = subImage->mImageSubresourceInfo;
        ...
        if (mFramebuffer->hasDepthAttachment())
        {
            ...
            for (uint32_t j = 0; j < fbAttachment.image->mImageInfo.numSubresourceInfos; j++)
            {
                VulkanImageSubresource* subImage = fbAttachment.image->getSubresource(0, fbAttachment.image->mImageInfo.subresourceInfoIdx + j);
                ImageSubresourceInfo& entry = subImage->mImageSubresourceInfo;
                entry.isFBAttachment = false;
            }
        }
...
void VulkanCmdBuffer::clearViewport(uint32_t buffers, const irr::video::SColor& color, float depth, UINT16 stencil, UINT8 targetMask)
{
    core::recti area(
        ...
        (uint32_t)(mViewport.UpperLeftCorner.Y),
        (uint32_t)(mViewport.LowerRightCorner.X),
        (uint32_t)(mViewport.LowerRightCorner.Y));
...
void VulkanCmdBuffer::setVertexBuffers(uint32_t index, VulkanBuffer** buffers, uint32_t numBuffers, VkDeviceSize* offset)
{
    ...
    vkCmdBindVertexBuffers(mCmdBuffer, index, numBuffers, mVertexBuffersTemp, offset);
}

void VulkanCmdBuffer::setIndexBuffer(VulkanBuffer* buffer, E_INDEX_TYPE type, VkDeviceSize offset)
{
...
void VulkanCmdBuffer::bindDynamicStates(bool forceAll)
{
    if (mViewportRequiresBind || forceAll)
    {
        ...
        _viewport.x = mViewport.UpperLeftCorner.X;
        _viewport.y = mViewport.UpperLeftCorner.Y;
        _viewport.width = mViewport.getWidth();
        _viewport.height = mViewport.getHeight();
        _viewport.minDepth = 0.0f;
        _viewport.maxDepth = 1.0f;

        //if (memcmp(&_viewport, &viewport, sizeof(VkViewport)))
        //{
            viewport = _viewport;
    ...
    if (mScissorRequiresBind || forceAll)
    {
        ...
        {
            _scissorRect.offset.x = mScissor.UpperLeftCorner.X;
            _scissorRect.offset.y = mScissor.UpperLeftCorner.Y;
            _scissorRect.extent.width = mScissor.getWidth();
            _scissorRect.extent.height = mScissor.getHeight();
        }
        else
        {
            _scissorRect.offset.x = 0;
            _scissorRect.offset.y = 0;
...
{
    if (mQueuedLayoutTransitions.empty() && mLayoutTransitionBarriersTemp.empty())
        return;
    ...
    auto createLayoutTransitionBarrier = [&](VulkanImage* image, ImageInfo& imageInfo)
    {
        ...
        {
            VulkanImageSubresource* subImage = image->getSubresource(0, imageInfo.subresourceInfoIdx + i);
            ImageSubresourceInfo& subresourceInfo = subImage->mImageSubresourceInfo;
    ...
    for (auto& entry : mQueuedLayoutTransitions)
    {
        ...
        ImageInfo& imageInfo = entry->mImageInfo; //mImageInfos[imageInfoIdx];

        createLayoutTransitionBarrier(entry, imageInfo);
    }
...
VkImageLayout VulkanCmdBuffer::getCurrentLayout(VulkanImage* image, const VkImageSubresourceRange& range, bool inRenderPass)
{
    ...
    if (image->mUseHandle.mPoolMask & 1)
    {
        ImageSubresourceInfo& entry = subresource->mImageSubresourceInfo;
...
void VulkanCmdBuffer::registerResource(CVulkanDeviceResource* res, VulkanUseFlag flags)
{
    ...
    ResourceUseHandle& useHandle = res->mUseHandle;

    if (useHandle.mPoolMask & 1)
    {
        assert(!useHandle.used);
...
void VulkanCmdBuffer::registerResource(VulkanImage* res, const VkImageSubresourceRange& range, VkImageLayout newLayout, VkImageLayout finalLayout, VulkanUseFlag flags, ResourceUsage usage)
{
    ...
    auto registerSubresourceInfo = [&](const VkImageSubresourceRange& range)
    {
        ...
        {
            VulkanImageSubresource* subImage = res->getSubresource(0, range.baseMipLevel + i);
            if (!subImage)
                continue;
    ...
    if ((res->mUseHandle.mPoolMask & 1) == 0) // New element
    {
        ...
        res->mUseHandle.mPoolMask |= 1;
        res->mUseHandle.used = false;
...
void VulkanCmdBuffer::registerResource(VulkanBuffer* res, VkAccessFlags accessFlags, VulkanUseFlag flags)
{
    ...
    if (res->mUseHandle.mPoolMask & 1)
    {
        assert(!res->mUseHandle.used);
        res->mUseHandle.flags |= flags;
        res->mBufferInfo.accessFlags |= accessFlags;
        ...
        if (res->mBufferInfo.needsBarrier && (isShaderRead || isShaderWrite))
        {
            ...
            res->mBufferInfo.needsBarrier = isShaderWrite;
        }
        return;
    }
    ...
    if (!isTransfer)
    {
        ...
        else
        {
            if (subresourceInfo.currentLayout != subresourceInfo.requiredLayout)
                mQueuedLayoutTransitions.push_back(image);
        }
    }
    else
    {
...
ImageSubresourceInfo& VulkanCmdBuffer::findSubresourceInfo(VulkanImage* image, uint32_t face, uint32_t mip)
{
    assert(image->mUseHandle.mPoolMask & 1);

    VulkanImageSubresource* subResource = image->getSubresource(face, mip);
    if (subResource)
        return subResource->mImageSubresourceInfo;
