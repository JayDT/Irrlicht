...
VulkanFramebuffer::VariantKey::VariantKey(RenderSurfaceMaskBits loadMask, RenderSurfaceMaskBits readMask, u32 clearMask)
...
size_t VulkanFramebuffer::VariantKey::HashFunction::operator()(const VariantKey& v) const
{
    ...
    hash = VK_GUID_HASHER(v.readMask);
    hash ^= VK_GUID_HASHER(v.loadMask) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
    hash ^= VK_GUID_HASHER(v.clearMask) + 0x9e3779b9 + (hash << 6) + (hash >> 2);
...
VulkanFramebuffer::~VulkanFramebuffer()
{
    ...
    {
        vkDestroyFramebuffer(device, entry.second.framebuffer, VulkanDevice::gVulkanAllocator);
        vkDestroyRenderPass(device, entry.second.renderPass, VulkanDevice::gVulkanAllocator);
    }
}
...
VulkanFramebuffer::Variant VulkanFramebuffer::createVariant(RenderSurfaceMaskBits loadMask, RenderSurfaceMaskBits readMask, u32 clearMask) const
{
    for (uint32_t i = 0; i < mNumColorAttachments; i++)
    {
        ...
        if (clearMask & (1 << attachment.index))
        {
            attachmentDesc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
            attachmentDesc.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        }
        else if (loadMask & ((RenderSurfaceMaskBits)(1 << attachment.index)))
        {
    ...
    if (mHasDepth)
    {
        ...
        else if (loadMask & (RT_DEPTH) || loadMask & (RT_STENCIL))
        {
            attachmentDesc.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
        ...
        //if (readMask & (RT_DEPTH) && attachmentDesc.loadOp == VK_ATTACHMENT_LOAD_OP_DONT_CARE && attachmentDesc.stencilLoadOp == VK_ATTACHMENT_LOAD_OP_DONT_CARE)
        //{
...
uint32_t VulkanFramebuffer::getNumClearEntries(u32 clearMask) const
{
    ...
    for (int32_t i = _MAX_MULTIPLE_RENDER_TARGETS - 1; i >= 0; i--)
    {
