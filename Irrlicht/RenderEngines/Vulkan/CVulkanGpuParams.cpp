...
void VulkanGpuParams::initialize()
{
    ...
    for (irr::u8 i = 0; i < _MAX_DEVICES; i++)
    {
        ...
        mNumDeviceData = numSets;
        mPerDeviceData[i] = new PerSetData[numSets];
...
void irr::video::VulkanGpuParams::reset()
{
    ...
    //{
    //    if (mPerDeviceData[i])
    //        for (u32 s = 0; s < mNumDeviceData; ++s)
    //            mPerDeviceData[i][s].mSetsDirty = true;
    //}
}
...
void VulkanGpuParams::setParamBlockBuffer(u32 set, u32 slot, const CVulkanHardwareBuffer* paramBlockBuffer)
{
    ...
    for (u32 i = 0; i < _MAX_DEVICES; i++)
    {
        ...
        PerSetData& perSetData = mPerDeviceData[i][set];
...
void VulkanGpuParams::setTexture(u32 set, u32 slot, const ITexture* texture, const TextureSurface& surface)
{
    ...
    for (u32 i = 0; i < _MAX_DEVICES; i++)
    {
        ...
        PerSetData& perSetData = mPerDeviceData[i][set];
        if (perSetData.writeInfos[bindingIdx].texture == texture)
            continue;

        perSetData.writeInfos[bindingIdx].texture = texture;
...
void VulkanGpuParams::setBuffer(u32 set, u32 slot, const CVulkanHardwareBuffer* buffer)
{
    ...
    for (u32 i = 0; i < _MAX_DEVICES; i++)
    {
        ...
        bool useView = u8(writeSetInfo.descriptorType) != u8(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER);
        if (useView)
        {
            VkBufferView bufferView;
            if (bufferRes != nullptr)
...
{
    return mNumDeviceData;
}

bool VulkanGpuParams::UpdateDescriptors(VulkanCmdBuffer& buffer, VkDescriptorSet* sets)
{
    u32 deviceIdx = buffer.getDeviceIdx();
     
    PerSetData* perDeviceData = mPerDeviceData[deviceIdx];
    if (!perDeviceData)
        return false;
    ...
    {
        PerSetData& perSetData = perDeviceData[i];
    ...
    for (u32 i = 0; i < numSets; i++)
    {
        VulkanDescriptorSet* set = perDeviceData[i].latestSet;
    
        if (sets[i] != set->getHandle())
        {
