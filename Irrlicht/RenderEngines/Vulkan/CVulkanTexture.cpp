...
CVulkanTexture::CVulkanTexture(CVulkanDriver* driver, const core::dimension2d<u32>& size, const io::path& name,
    const ECOLOR_FORMAT format, u32 arraySlices, u32 sampleCount, u32 sampleQuality, E_TEXTURE_TYPE type)
    : ITexture(name)
    ...
    , Image(nullptr)
{
    ...
    if (IImage::isDepthFormat(format))
        UsageFlags |= IImage::isStencilFormat(format) ? ETCF_USAGE_DEPTHSTENCIL : ETCF_USAGE_DEPTH;
    else
        UsageFlags |= ETCF_USAGE_RENDERTARGET;

    bool success = InitializeColorFormat(UsageFlags, format);

    createTexture(UsageFlags, Image, type);
}

irr::video::CVulkanTexture::CVulkanTexture(CVulkanDriver* driver, VulkanImage* image, const core::dimension2d<u32>& size, const ECOLOR_FORMAT format)
    : ITexture("")
    //, CVulkanDeviceResource(static_cast<CVulkanDriver*>(driver))
    , TextureSize(size)
    , ImageSize(size)
...
CVulkanTexture::CVulkanTexture(IImage* image, CVulkanDriver* driver, u32 flags, const io::path& name,
    u32 arraySlices, void* mipmapData, u32 sampleCount, u32 sampleQuality, E_TEXTURE_TYPE type)
    : ITexture(name)
    //, CVulkanDeviceResource(static_cast<CVulkanDriver*>(driver))
    , TextureSize(0, 0)
    , ImageSize(0, 0)
...
void * irr::video::CVulkanTexture::lock(E_TEXTURE_LOCK_MODE mode, u32 mipmapLevel, u32 arraySlice)
{
    ...
    if (needRead) // If reading, we need to copy the current contents of the image to the staging buffer
    {
        ...
        if ((UsageFlags & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
...
bool irr::video::CVulkanTexture::createMipMaps(u32 level)
{
    ...
    else // Software mode (Experimental)
    {
        ...
        do
        {
            ...
            size_t imageSize = IImage::getMemorySize(size.Width, size.Height, 1, getColorFormat());

            void* data = lock(E_TEXTURE_LOCK_MODE::ETLM_WRITE_ONLY, level, 0);
            Image->copyToScaling(data, size.Width, size.Height, getColorFormat(), imageSize);
            unlock();
...
{
    // get color format
    if (colorFormat == ECF_R8G8B8) // most Vulkan drivers don't support 24bit formats
        mInternalFormats[0] = ECF_B8G8R8A8;  // upgrade to 32bit format
    else
...
bool irr::video::CVulkanTexture::createTexture(u32 flags, IImage * image, E_TEXTURE_TYPE type)
{
    ...
    else if ((flags & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
    {
    ...
    if ((flags & ETCF_USAGE_DYNAMIC) != 0) // Attempt to use linear tiling for dynamic textures, so we can directly map and modify them
    {
        ...
        if (type == E_TEXTURE_TYPE::e2D && QualityOfSample <= 1 && NumberOfMipLevels <= 1 &&
...
{
    uint64_t size = 0;
    for (int i = 0; i < image->GetMipLevelCount(); ++i)
        size += image->getImageDataSizeInBytes(i);

    createTextureBuffer(false, size);

    return true;
}

bool irr::video::CVulkanTexture::createTextureBuffer(bool readable, uint64_t size /*= 0*/)
{
    if (mStagingBuffer)
    {
...
void irr::video::CVulkanTexture::regenerateMipMapLevels(IImage* image)
{
    ...
    , mNumMipLevels(desc.numMipLevels)
{
    ...
    else if ((mUsage & ETCF_USAGE_DEPTH) != 0)
    {
        mFramebufferMainView = createView(completeSurface, getAspectFlags());
...
{
    if (framebuffer && (mUsage & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
...
VkImageView VulkanImage::getView(const TextureSurface& surface, bool framebuffer) const
{
    for (auto& entry : mImageInfos)
    {
        ...
        {
            if ((mUsage & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) == 0)
                return entry.view;
            else
            {
...
VkImageLayout VulkanImage::getOptimalLayout() const
{
    ...
    else if ((mUsage & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
...
VkImageAspectFlags VulkanImage::getAspectFlags() const
{
    if ((mUsage & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
    {
        if (mUsage & ETCF_USAGE_DEPTHSTENCIL)
