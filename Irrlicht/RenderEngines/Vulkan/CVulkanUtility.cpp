...
uint64_t VK_GUID_HASHER(uint64_t u)
{
...
ECOLOR_FORMAT VulkanUtility::getClosestSupportedPixelFormat(VulkanDevice& device, ECOLOR_FORMAT format, E_TEXTURE_TYPE texType, int usage, bool optimalTiling, bool hwGamma)
{
    ...
    if ((usage & (ETCF_USAGE_DEPTHSTENCIL | ETCF_USAGE_DEPTH)) != 0)
    ...
    {
        if ((usage & ETCF_USAGE_DEPTHSTENCIL) != 0 || (usage & ETCF_USAGE_DEPTH) != 0)
        {
            bool hasStencil = format == ECOLOR_FORMAT::ECF_D24S8 || format == ECOLOR_FORMAT::ECF_D32_S8X24;
            ...
            if (hasStencil)
            {
                if (isSupported(VK_FORMAT_D32_SFLOAT_S8_UINT))
                    ...
                    format = ECOLOR_FORMAT::ECF_D24S8;
    ...
    VkImageSubresourceRange* output, uint32_t& numAreas)
{
    ...
    int32_t leftCut = cutWith.baseArrayLayer - toCut.baseArrayLayer;
    int32_t rightCut = (cutWith.baseArrayLayer + cutWith.layerCount) - toCut.baseArrayLayer;

    if (leftCut > 0 && leftCut < (int32_t)(toCut.baseArrayLayer + toCut.layerCount))
    {
    ...
    VkImageSubresourceRange* output, uint32_t& numAreas)
{
    ...
    int32_t topCut = cutWith.baseMipLevel - toCut.baseMipLevel;
    int32_t bottomCut = (cutWith.baseMipLevel + cutWith.levelCount) - toCut.baseMipLevel;

    if (topCut > 0 && topCut < (int32_t)(toCut.baseMipLevel + toCut.levelCount))
    {
        output[numAreas] = toCut;
        VkImageSubresourceRange& range = output[numAreas];
