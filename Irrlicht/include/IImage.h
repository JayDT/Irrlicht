...
public:
	...
	virtual const core::dimension2d<u32>& getDimension(u8 miplevel = 0) const = 0;
	...
	virtual u32 getImageDataSizeInPixels(u8 miplevel = 0) const = 0;
    ...
    inline static bool isDepthFormat(const ECOLOR_FORMAT format)
    {
        switch (format)
        {
            case ECF_D16:
    ...
    inline static bool isStencilFormat(const ECOLOR_FORMAT format)
    {
        switch (format)
        {
            case ECF_D24S8:
    ...
    inline static u32 getMemorySize(u32 width, u32 height, u32 depth, ECOLOR_FORMAT format)
    {
        if (isCompressedFormat(format))
        {
            switch (format)
            {
                ...
                case ECF_DXT1:
                case ECF_DXT4:
                    return ((width + 3) / 4)*((height + 3) / 4) * getBitsPerPixelFromFormat(format) * depth;
                case ECF_DXT2:
                case ECF_DXT3:
                case ECF_DXT5:
                    return ((width + 3) / 4)*((height + 3) / 4) * getBitsPerPixelFromFormat(format) * depth;
