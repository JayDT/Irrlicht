...
enum E_TEXTURE_CREATION_FLAG
{
    ...
    /** Texture used as a depth/stencil buffer by the GPU. */
    ETCF_USAGE_DEPTHSTENCIL = 0x00002000,
...
public:
	...
	//! constructor
	ITexture(const io::path& name, E_TEXTURE_TYPE type = E_TEXTURE_TYPE::e2D)
        : NamedPath(name)
        , Type(type)
	{
	}
	...
	virtual void regenerateMipMapLevels(IImage* image = nullptr) = 0;
    ...
    static bool checkFormat(ECOLOR_FORMAT& format, E_TEXTURE_TYPE texType, int usage)
    {
        ...
        {
            if (IImage::isDepthFormat(format) && IImage::isStencilFormat(format))
                return true;
        ...
        //// Depth only supports depth formats
        if ((usage & ETCF_USAGE_DEPTH) != 0)
        {
            if (IImage::isDepthFormat(format) && !IImage::isStencilFormat(format))
                return true;

            format = ECOLOR_FORMAT::ECF_D32;
            return false;
        }
        ...
        {
            if (/*IImage::isRenderTargetOnlyFormat(format) && */ !IImage::isDepthFormat(format) && !IImage::isCompressedFormat(format))
                return true;
        ...
        {
            if (!IImage::isDepthFormat(format) && !IImage::isCompressedFormat(format))
                return true;
