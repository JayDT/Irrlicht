...
Log::Log()
    : logfile(nullptr)
    , dberLogfile(nullptr)
    , m_logLevel(System::LogLevel::LOG_LVL_MINIMAL)
    , m_colored(true)
    , m_includeTime(true)
{
    Initialize();
...
void Log::InitColors(const std::string& str)
{
    ...
    m_colors[LogType::LogWarning] = Color::YELLOW;
...
void Log::WriteToConsole(const char * str, LogType logType, bool is_inline)
{
    if (!str)
        return;
...
#define ApplyLogCVaradicalArg(fmt, buf)         \
        va_list ap;                             \
        va_start(ap, fmt);                      \
...
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogNormal);
}

void Log::WriteErrorLine(const char * fmt, ...)
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogError);
}

void Log::WriteErrorDbLine(const char * fmt, ...)
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogDBError);
}

void Log::WriteDebugLine(const char * fmt, ...)
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogDebug);
}

void Log::WriteDetailLine(const char * fmt, ...)
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogDetails);
}

void Log::WriteWarnLine(const char * fmt, ...)
{
    char buf[2048];
    ApplyLogCVaradicalArg(fmt, buf);
    WriteToConsole(buf, LogType::LogWarning);
}