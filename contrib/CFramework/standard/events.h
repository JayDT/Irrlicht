...
namespace System
{
    namespace Events
    {
        ...
        {
            virtual ~EventArg() {}
            virtual std::string ToString() const { return "EventArg"; }
            virtual int getTypeId() override { return 0; }
        };

        static EventArg sEventArg;

        struct IEventHook : System::EnableSharedFromThisEx<IEventHook>
        {
            virtual ~IEventHook() {}
        ...
        struct IPropertyEvent
        {
			virtual ~IPropertyEvent() = 0;
        };
