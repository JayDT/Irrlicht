...
void System::FileSystem::GetFileList(std::filesystem::path const & path, TFileList & result_set, bool recurisve, const std::regex * filter)
{
    ...
    if (fs::exists(path) && fs::is_directory(path))
    {
        for (fs::directory_iterator dir_iter(path); dir_iter != end_iter; ++dir_iter)
        {
            ...
            {
                std::cmatch what;
                if (!filter || std::regex_search(dir_iter->path().filename().string().c_str(), what, *filter))
...
void System::FileSystem::CreateDir(std::filesystem::path const & path)
{
