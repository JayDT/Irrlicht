...
int main()
{
	...
	*/
    ...
    //core::stringw title;
    //title += "Hello World! This is the Irrlicht renderer with";
    //title += driver->getName();
    //
	//guienv->addStaticText(title.c_str(),
	//	rect<s32>(10,10,260,22), true)->setEnabled(false);
    //
    //guienv->addStaticText(L"FPS: ",
    //    rect<s32>(10, 30, 60, 52), true)->setEnabled(false);

    auto fpsText = guienv->addStaticText(L"0",
        rect<s32>(65, 30, 260, 52), true);
    fpsText->setEnabled(false);
    fpsText->setDrawBorder(false);
	...
	*/
    ...
    node->getMesh()->getMeshBuffer(0)->setHardwareMappingHint(irr::scene::EHM_DYNAMIC, scene::EBT_VERTEX);
    //node->getMesh()->getMeshBuffer(0)->setHardwareMappingHint(irr::scene::EHM_DYNAMIC, scene::EBT_INDEX);
	/*
	...
	*/
	...
	while(device->run())
	{
		...
		*/
		driver->beginScene(true, true, SColor(255, 80, 80, 80));
        ...
        if (++count > 2000)
        {
            ...
            {
                lastFps = fps;
                fpsText->setText(std::to_wstring(fps).c_str());
            }
        }
	}
